...
function otp(message, key, mode, keyRepetition) {
	...
	if (message === "" || key === "") {
		error = "Both The message and the key must not be be empty.";
	...
	if (mode == "encrypt") {
		...
		if (!isNaN(message)) {
			...
			error = "When decrypting, the message must only contain numbers.";
			console.log("[OneTimePad.js] " + error);
			return error;
		}
	}
	...
	if (codeKey.length < codeMessage.length) {
		...
		// Otherwise, if the key is too short, fail with an error.
			...
			error = "The key is shorter than the message.";
			console.log("[OneTimePad.js] " + error);
			return error;
		}
	}
